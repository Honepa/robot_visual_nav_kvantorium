...
def get_state_drv():
    if request.headers['X_REQUESTED_WITH'] == 'XMLHttpRequest':
        resp = requests.get(f'http://127.0.0.1:4998/state/get')
        j = resp.json()
        data = {
            'echo': True, 
            'action': 'state', 
...
def robot_get_coords(last_coordinates=(0,0)):
    if request.headers['X_REQUESTED_WITH'] == 'XMLHttpRequest':
        ...
        # })
        resp = requests.post(f'http://127.0.0.1:4997/coords/calc', {'last_coords': last_coordinates})
        coords = resp.json()
        coords_xy = (int(coords[0]/100), int(coords[1]/100))
        return jsonify(coords_xy), 200
    else:
        return jsonify(message="Method Not Allowed"), 405
