...
class PwmProcess(Process):
    def __init__(self, name, gpio, pins, q_pwm_value, frequency=50, debug=False, debug_delay=1000):
        ...
        self.__gpio.setup([x for p in list(pins.values()) for x in p], self.__gpio.OUT, initial=self.__gpio.LOW)


    def __do_debug__(self):
        self.__loop_count += 1
        if self.__loop_count == self.__debug_delay:
            print(f'{self.__name} pwm {self.__pwm}', file=sys.stderr)
            self.__loop_count = 0
