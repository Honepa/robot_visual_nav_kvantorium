...
if __name__ == '__main__':
    img_sample = cv.imread('samples/a.jpeg', 0)
    kp1, des1 = sift.detectAndCompute(img_sample,None)

    for dist, ext in [ x.split('.') for x in glob(f'imgs/*.{FILE_EXT}') ]:
        img = cv.imread(f'{dist}.{ext}')
        kp2, des2 = sift.detectAndCompute(img, None)
        matches = flann.knnMatch(des1, des2, k=2)

        dist = int(dist.split('/')[1]) # mm
        ...
        if len(good) > MIN_MATCH_COUNT:
            ...
            print(dist, W_mm * w_i/w_0, H_mm * h_i/h_0)

            X += [dist]
            W += [W_mm * w_0/w_i] # mm / px
            H += [H_mm * h_0/h_i]
